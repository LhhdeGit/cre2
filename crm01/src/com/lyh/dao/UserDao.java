...
//              System.out.println(rs.getString("username"));  //拿到了每个数据的行
    ...
    public List<User> selectAllByParam(Map map){
        ...
        String page = (String) map.get("page");  //接收前端的参数，放入到map中，这里直接放到map中
        ...
        //如果说type不为空 real_name不为空  如果说real_name不为空  如果说username不为空
        ...
        Connection connection = DBHelper.getConnection();
        //2.写sql语句
        String sql = "select * from t_user where 1=1 "; //where保持恒等 因为有多余的and
